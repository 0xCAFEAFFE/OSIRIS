...
/*
    ...
    Required by the OSIRIS project. Possibly useful for other applications too.
    
    Issue 1:
    
    If CH340 is not supplied via USB (battery power) its TX pin is in a unstable state
    That will sometimes but not always trigger a RX frame error in the bootloader -> inconsistent timing behavior at startup
    -> Solution:
        Measure VUSB, if USB is not connected, BL has nothing to do anyways -> boot into application ASAP
   
    Issue 2:
    
    The AVR internal RC oscillator frequency can deviate by as much as 10% from the nominal frequency of 8MHz.
    This is a problem for UART since the CH340 may also have a frequency error of up to 1%.
    -> Solution:
        The internal RC oscillator is calibrated against the external 32kHz XTAL oscillator
        and a more accurate UBRR value is calculated by the application and stored to EEPROM.
        Bootloader can now use that UBRR value and communicate without errors
*/
...
// enable UART0, double speed mode 
UART_SRA = _BV(U2X0);
UART_SRB = _BV(RXEN0) | _BV(TXEN0);
UART_SRC = _BV(UCSZ00) | _BV(UCSZ01);

while(!eeprom_is_ready());
uint8_t ubrr = eeprom_read_byte(BOOT_UBRR_EEP_ADDR);

// CH340 allows <2% error rate - internal RC oscillator ranges from 7.6 - 8.4MHz
if(ubrr >= BOOT_UBRR_MIN && ubrr <= BOOT_UBRR_MAX)
    UART_SRL = ubrr;    // load from eeprom
else
